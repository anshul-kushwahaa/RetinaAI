...
class TrainingProgress(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(f"\n{'='*60}")
        print(f"  EPOCH {current:02d}/{self.total_epochs}  [{bar}] {pct}%{tag}")
        print(f"{'='*60}")
...
def preprocess_image(image_id, augment=False):
...
def main():
    ...
    callbacks_p1 = [
        TrainingProgress(total_epochs=20),
    ...
    # ── Phase 2: Fine-tune top 50 layers of DenseNet ─────────────
    ...
    callbacks_p2 = [
        TrainingProgress(total_epochs=30),
        ModelCheckpoint(MODEL_SAVE_PATH, save_best_only=True, monitor="val_accuracy", verbose=0),
